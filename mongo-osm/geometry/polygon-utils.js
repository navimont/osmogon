...
function reduce(mem) {
    ...
    for (var i=1; i<mem.length; i++) {
        ...
        if (Geomath.same (m0[m0.length-1], m1[m1.length-1])) {
            m1.reverse();
        } else if (Geomath.same (m0[0], m1[0])) {
            m0.reverse();
        } else if (Geomath.same (m0[0], m1[m1.length-1])) {
            m0.reverse();
            m1.reverse();
        }
        // last node in m0 is the same as first node in m1
        if (Geomath.same(m0[m0.length-1], m1[0])) {
