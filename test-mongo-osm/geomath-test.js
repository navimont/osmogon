...
exports.samePoint = function(test) {
    var p1 = [1,1];
    var p2 = [1.0000000001,0.99999999999];
    test.ok(Geomath.same(p1,p2));
    test.done();
}

exports.notQuiteTheSamePoint = function(test) {
    var p1 = [1,1];
    var p2 = [1.000001,0.999999];
    test.equals(false, Geomath.same(p1,p2));
    test.done();
}
...
exports.intersection = {
    setUp: function(callback) {
        // line parallel to x-axis at y=1
        this.p0 = [-1, 1];
        this.p1 = [1, 1];
        callback();
    },
    intersectBetweenPoints: function(test) {
        // ray from 0,0 in y direction
        test.ok(Geomath.rayIntersectsBetweenPoints([0,0], Vector.create([0,1]), this.p0, this.p1));
        test.ok(Geomath.rayIntersectsBetweenPoints([0,0], Vector.create([0,1]), this.p1, this.p0));
        test.done();
    },
    intersectOnPoint: function(test) {
        // ray goes through p1
        test.ok(Geomath.rayIntersectsBetweenPoints([0,1], Vector.create([0,1]), this.p0, this.p1));
        test.ok(Geomath.rayIntersectsBetweenPoints([0,0], Vector.create([2,2]), this.p1, this.p0));
        test.done();
    },
    intersectOutsidePointsFurther: function(test) {
        // ray passes further away
        var res1 = Geomath.rayIntersectsBetweenPoints([1,0], Vector.create([1,1]), this.p0, this.p1);
        var res2 = Geomath.rayIntersectsBetweenPoints([0,0], Vector.create([2,1.5]), this.p1, this.p0);
        test.equals(false, res1);
        test.equals(false, res2);
        test.done();
    },
    intersectCoincidentLines: function(test) {
        // ray covers line
        test.ok(Geomath.rayIntersectsBetweenPoints([0,1], Vector.create([3,1]), this.p0, this.p1));
        test.done();
    },
    intersectParallelLines: function(test) {
        // ray parallel to p0->p1
        test.equals(false, Geomath.rayIntersectsBetweenPoints([0,0], Vector.create([1,0]), this.p0, this.p1));
        test.done();
    }
}
...
exports.pointInPolygon = {
    setUp: function(callback) {
        this.polygon = [[1,1],[3,1],[2,3],[1,1]];
        callback();
    },
    pointInPolygon: function(test) {
        test.ok(Geomath.isPointInPolygon(this.polygon, [2,2]));
        test.done();
    },
    pointOnEdge: function(test) {
        test.ok(Geomath.isPointInPolygon(this.polygon, [2,1]));
        test.done();
    },
    pointOutsidePolygon: function(test) {
        test.equals(false, Geomath.isPointInPolygon(this.polygon, [4,1]));
        test.done();
    }
}
